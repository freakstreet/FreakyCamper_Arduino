...
static void adb_printMessage(adb_message * message)
{
	...
	{
	case A_OKAY:
		Serial.println("OKAY message");// [%lx] %ld %ld\n", message->command, message->arg0, message->arg1);
		break;
	case A_CLSE:
		Serial.println("CLSE message ");//[%lx] %ld %ld\n", message->command, message->arg0, message->arg1);
		break;
	case A_WRTE:
		Serial.println("WRTE message" );//[%lx] %ld %ld, %ld bytes\n", message->command, message->arg0, message->arg1, message->data_length);
		break;
	case A_CNXN:
		Serial.println("CNXN message ");//[%lx] %ld %ld\n", message->command, message->arg0, message->arg1);
		break;
	case A_SYNC:
		Serial.println("SYNC message ");//[%lx] %ld %ld\n", message->command, message->arg0, message->arg1);
		break;
	case A_OPEN:
		Serial.println("OPEN message");// [%lx] %ld %ld\n", message->command, message->arg0, message->arg1);
		break;
	default:
		Serial.println("WTF message ");//[%lx] %ld %ld\n", message->command, message->arg0, message->arg1);
		break;
	}
}
#endif
...
#ifdef DEBUG
	Serial.print("OUT << "); adb_printMessage(&message);
#endif
...
boolean ADB::pollMessage(adb_message * message, boolean poll)
{
	...
	{
#ifdef DEBUG
		Serial.print("Broken message, magic mismatch");// %d bytes\n", bytesRead);
		return false;
#endif
	}
